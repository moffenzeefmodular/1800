...
#include "AudioSampleStar.h"
#include "AudioSample0.h"
#include "AudioSamplePound.h"
...
void loop() {
  ...
  if (currentState1 < lastState1) {
    playMem1.play(AudioSample1); // Play sound
  }

  if (currentState2 < lastState2) {
    playMem2.play(AudioSample2); // Play sound
  }

  if (currentState3 < lastState3) {
    playMem3.play(AudioSample3); // Play sound
  }

  if (currentState4 < lastState4) {
    playMem4.play(AudioSample4); // Play sound
  }

  if (currentState5 < lastState5) {
    playMem5.play(AudioSample5); // Play sound
  }

  if (currentState6 < lastState6) {
    playMem6.play(AudioSample6); // Play sound
  }

  if (currentState7 < lastState7) {
    playMem7.play(AudioSample7); // Play sound
  }

  if (currentState8 < lastState8) {
    playMem8.play(AudioSample8); // Play sound
  }

  if (currentState9 < lastState9) {
    playMem9.play(AudioSample9); // Play sound
  }

  if (currentState10 < lastState10) {
    playMem10.play(AudioSampleStar); // Play sound
  }

  if (currentState11 < lastState11) {
    playMem11.play(AudioSample0); // Play sound
  }

  if (currentState12 < lastState12) {
    playMem12.play(AudioSamplePound); // Play sound
  }
