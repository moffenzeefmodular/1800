...
#include "AudioSampleBongo1.h"
#include "AudioSampleBongo2.h"
#include "AudioSampleBongo3.h"
#include "AudioSampleBongo4.h"
...
void loop() {
  ...
  if (currentState3 < lastState3) {
    playMem3.play(AudioSampleSnare2); // Play sound
  }

  if (currentState4 < lastState4) {
    playMem4.play(AudioSampleWood); // Play sound
  }

  if (currentState5 < lastState5) {
    playMem5.play(AudioSampleWood2); // Play sound
  }

  if (currentState6 < lastState6) {
    playMem6.play(AudioSampleBongo1); // Play sound
  }

  if (currentState7 < lastState7) {
    playMem7.play(AudioSampleBongo2); // Play sound
  }

  if (currentState8 < lastState8) {
    playMem8.play(AudioSampleBongo3); // Play sound
  }

  if (currentState9 < lastState9) {
    playMem9.play(AudioSampleBongo4); // Play sound
  }

  if (currentState10 < lastState10) {
    playMem10.play(AudioSampleHhc); // Play sound
  }

  if (currentState11 < lastState11) {
    playMem11.play(AudioSampleHho); // Play sound
  }

  if (currentState12 < lastState12) {
    playMem12.play(AudioSampleCrash); // Play sound
  }
