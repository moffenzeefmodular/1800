...
void loop() {
  ...
  if (currentState2 < lastState2) {
    playMem2.play(AudioSampleSnare3); // Play sound
  }

  if (currentState3 < lastState3) {
    playMem3.play(AudioSampleStick); // Play sound
  }

  if (currentState4 < lastState4) {
    playMem4.play(AudioSampleClap); // Play sound
  }

  if (currentState5 < lastState5) {
    playMem5.play(AudioSampleHhc); // Play sound
  }

  if (currentState6 < lastState6) {
    playMem6.play(AudioSampleHho); // Play sound
  }

  if (currentState7 < lastState7) {
    playMem7.play(AudioSampleCongahi); // Play sound
  }

  if (currentState8 < lastState8) {
    playMem8.play(AudioSampleCongalo); // Play sound
  }

  if (currentState9 < lastState9) {
    playMem9.play(AudioSampleCabasa); // Play sound
  }

  if (currentState10 < lastState10) {
    playMem10.play(AudioSampleCowbell); // Play sound
  }

  if (currentState11 < lastState11) {
    playMem11.play(AudioSampleAgogo); // Play sound
  }

  if (currentState12 < lastState12) {
    playMem12.play(AudioSampleTamb); // Play sound
  }
