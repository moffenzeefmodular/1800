...
#include "AudioSampleCzchh.h"
#include "AudioSampleCzclap.h"
#include "AudioSampleCzcow.h"
#include "AudioSampleCzcrash.h"
#include "AudioSampleCzkick.h"
#include "AudioSampleCzohh.h"
#include "AudioSampleCzride.h"
#include "AudioSampleCzrim.h"
#include "AudioSampleCzsnare.h"
#include "AudioSampleCztoml.h"
#include "AudioSampleCztomm.h"
#include "AudioSampleCztomh.h"
...
void loop() {
  ...
  if (currentState1 < lastState1) {
    playMem1.play(AudioSampleCzkick); // Play sound
  }

  if (currentState2 < lastState2) {
    playMem2.play(AudioSampleCzsnare); // Play sound
  }

  if (currentState3 < lastState3) {
    playMem3.play(AudioSampleCzclap); // Play sound
  }

  if (currentState4 < lastState4) {
    playMem4.play(AudioSampleCzrim); // Play sound
  }

  if (currentState5 < lastState5) {
    playMem5.play(AudioSampleCzchh); // Play sound
  }

  if (currentState6 < lastState6) {
    playMem6.play(AudioSampleCzohh); // Play sound
  }

  if (currentState7 < lastState7) {
    playMem7.play(AudioSampleCztoml); // Play sound
  }

  if (currentState8 < lastState8) {
    playMem8.play(AudioSampleCztomm); // Play sound
  }

  if (currentState9 < lastState9) {
    playMem9.play(AudioSampleCztomh); // Play sound
  }

  if (currentState10 < lastState10) {
    playMem10.play(AudioSampleCzcow); // Play sound
  }

  if (currentState11 < lastState11) {
    playMem11.play(AudioSampleCzride); // Play sound
  }

  if (currentState12 < lastState12) {
    playMem12.play(AudioSampleCzcrash); // Play sound
  }
