...
#include "AudioSampleCabasa.h"
#include "AudioSampleHibongo.h"
#include "AudioSampleHitimba.h"
#include "AudioSampleHmconga.h"
#include "AudioSampleHoconga.h"
#include "AudioSampleLobongo.h"
#include "AudioSampleLoconga.h"
#include "AudioSampleLotimba.h"
#include "AudioSampleMaracas.h"
#include "AudioSampleQuijada.h"
#include "AudioSampleWhistel.h"
#include "AudioSampleWhistle.h"
...
void loop() {
  ...
  if (currentState1 < lastState1) {
    playMem1.play(AudioSampleLoconga); // Play sound
  }

  if (currentState2 < lastState2) {
    playMem2.play(AudioSampleHmconga); // Play sound
  }

  if (currentState3 < lastState3) {
    playMem3.play(AudioSampleHoconga); // Play sound
  }

  if (currentState4 < lastState4) {
    playMem4.play(AudioSampleLobongo); // Play sound
  }

  if (currentState5 < lastState5) {
    playMem5.play(AudioSampleHibongo); // Play sound
  }

  if (currentState6 < lastState6) {
    playMem6.play(AudioSampleLotimba); // Play sound
  }

  if (currentState7 < lastState7) {
    playMem7.play(AudioSampleHitimba); // Play sound
  }

  if (currentState8 < lastState8) {
    playMem8.play(AudioSampleCabasa); // Play sound
  }

  if (currentState9 < lastState9) {
    playMem9.play(AudioSampleMaracas); // Play sound
  }

  if (currentState10 < lastState10) {
    playMem10.play(AudioSampleQuijada); // Play sound
  }

  if (currentState11 < lastState11) {
    playMem11.play(AudioSampleWhistel); // Play sound
  }

  if (currentState12 < lastState12) {
    playMem12.play(AudioSampleWhistle); // Play sound
  }
