...
#include "AudioSampleCongamid.h"
#include "AudioSampleCowbell.h"
#include "AudioSampleCymbal.h"
#include "AudioSampleHhclosed.h"
#include "AudioSampleHhopen.h"
...
void loop() {
  ...
  if (currentState5 < lastState5) {
    playMem5.play(AudioSampleHhclosed); // Play sound
  }

  if (currentState6 < lastState6) {
    playMem6.play(AudioSampleHhopen); // Play sound
  }

  if (currentState7 < lastState7) {
    playMem7.play(AudioSampleCongamid); // Play sound
  }

  if (currentState8 < lastState8) {
    playMem8.play(AudioSampleCongahi); // Play sound
  }

  if (currentState9 < lastState9) {
    playMem9.play(AudioSampleClave); // Play sound
  }

  if (currentState10 < lastState10) {
    playMem10.play(AudioSampleMaracas); // Play sound
  }
