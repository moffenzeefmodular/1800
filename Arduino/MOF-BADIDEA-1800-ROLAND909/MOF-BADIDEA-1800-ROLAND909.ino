...
#include "AudioSampleRide.h"
#include "AudioSampleRimshot.h"
#include "AudioSampleSnarelong.h"
#include "AudioSampleSnareshort.h"
#include "AudioSampleTomh.h"
#include "AudioSampleToml.h"
#include "AudioSampleTomm.h"
...
void loop() {
  ...
  if (currentState2 < lastState2) {
    playMem2.play(AudioSampleSnareshort); // Play sound
  }

  if (currentState3 < lastState3) {
    playMem3.play(AudioSampleSnarelong); // Play sound
  }

  if (currentState4 < lastState4) {
    playMem4.play(AudioSampleRimshot); // Play sound
  }

  if (currentState5 < lastState5) {
    playMem5.play(AudioSampleClap); // Play sound
  }

  if (currentState6 < lastState6) {
    playMem6.play(AudioSampleHhc); // Play sound
  }

  if (currentState7 < lastState7) {
    playMem7.play(AudioSampleHho); // Play sound
  }

  if (currentState8 < lastState8) {
    playMem8.play(AudioSampleToml); // Play sound
  }

  if (currentState9 < lastState9) {
    playMem9.play(AudioSampleTomm); // Play sound
  }
  ...
  if (currentState11 < lastState11) {
    playMem11.play(AudioSampleRide); // Play sound
  }
