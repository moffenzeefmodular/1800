...
#include "AudioSampleBongoh.h"
#include "AudioSampleBongol.h"
#include "AudioSampleCongal.h"
#include "AudioSampleCowbell.h"
#include "AudioSampleCymbal.h"
#include "AudioSampleGuiro.h"
#include "AudioSampleHh.h"
#include "AudioSampleHhm.h"
...
void loop() {
  ...
  if (currentState3 < lastState3) {
    playMem3.play(AudioSampleRimshot); // Play sound
  }

  if (currentState4 < lastState4) {
    playMem4.play(AudioSampleCongal); // Play sound
  }

  if (currentState5 < lastState5) {
    playMem5.play(AudioSampleBongol); // Play sound
  }

  if (currentState6 < lastState6) {
    playMem6.play(AudioSampleBongoh); // Play sound
  }

  if (currentState7 < lastState7) {
    playMem7.play(AudioSampleHh); // Play sound
  }

  if (currentState8 < lastState8) {
    playMem8.play(AudioSampleHhm); // Play sound
  }

  if (currentState9 < lastState9) {
    playMem9.play(AudioSampleGuiro); // Play sound
  }

  if (currentState10 < lastState10) {
    playMem10.play(AudioSampleTamb); // Play sound
  }

  if (currentState11 < lastState11) {
    playMem11.play(AudioSampleCowbell); // Play sound
  }

  if (currentState12 < lastState12) {
    playMem12.play(AudioSampleCymbal); // Play sound
  }
