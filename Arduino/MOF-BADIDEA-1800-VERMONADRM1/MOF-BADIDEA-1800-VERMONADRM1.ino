...
#include "AudioSampleCabasashort.h"
...
void loop() {
  ...
  if (currentState2 < lastState2) {
    playMem2.play(AudioSampleKick2); // Play sound
  }

  if (currentState3 < lastState3) {
    playMem3.play(AudioSampleSnare); // Play sound
  }

  if (currentState4 < lastState4) {
    playMem4.play(AudioSampleSnare2); // Play sound
  }

  if (currentState5 < lastState5) {
    playMem5.play(AudioSampleClick); // Play sound
  }

  if (currentState6 < lastState6) {
    playMem6.play(AudioSampleToml); // Play sound
  }

  if (currentState7 < lastState7) {
    playMem7.play(AudioSampleTomh); // Play sound
  }

  if (currentState8 < lastState8) {
    playMem8.play(AudioSampleHat); // Play sound
  }

  if (currentState9 < lastState9) {
    playMem9.play(AudioSampleHat2); // Play sound
  }

  if (currentState10 < lastState10) {
    playMem10.play(AudioSampleHat3); // Play sound
  }

  if (currentState11 < lastState11) {
    playMem11.play(AudioSampleCabasashort); // Play sound
  }

  if (currentState12 < lastState12) {
    playMem12.play(AudioSampleMulti); // Play sound
  }
